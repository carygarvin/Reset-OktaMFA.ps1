...
From the entries left (older than age treshold or blank), it will reset the MFA for each one of them and finally output in the current users '__My Documents__' folder the result in a CSV file titled with the execution time stamp followed by "___MFAResetReport.csv__".
