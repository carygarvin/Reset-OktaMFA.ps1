...
#  Upon launching the script, the user will be prompted to choose the CSV file with last MFA usage information.
...
#  From the entries left (older than age treshold or blank), it will reset the MFA for each one of them and finally output in the current user's 'My Documents' folder the result in a CSV file titled with the execution time stamp followed by "_MFAResetReport.csv".
#
#  There are 3 configurable variables (see lines 91 to 94 below) which need to be set by IT Administrator prior to using the present Script:
...
Else
	{
	write-host "No candidate input files have been found! Download at least one from the Okta Admin Portal by going to:"  -foregroundcolor "red"
	write-host "Admin/Reports/Reports/Multifactor Authentication/MFA Usage/Download CSV"
	write-host "Nothing to process. Aborting script!"  -foregroundcolor "red"
	}
